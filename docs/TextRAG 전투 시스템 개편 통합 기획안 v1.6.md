# TextRAG 전투 시스템 개편 통합 기획안 v1.6

- 기준 저장소: `J-Roc-H/text-rag`, `main` 기준 커밋 `5451ab1`
- 갱신일: 2026-09-25
- 변경 요약: v1.2 대비 보조 출처 ID 교차 후보를 266→293행으로 늘리고 미연결을 119→92행으로 줄였다. 신규 후보도 행동·시대 검증과 분리했으며, 행동 대조 완료는 105행으로 변하지 않았다. 중복·동일 표시명 variant를 별도 기록했다. 행동 대조 완료 수는 105행으로 변하지 않았다.
...
- 몬스터 DB 490행 전부 대장에 열거했다. pinned pre-re 자료까지 행동 대조한 행은 105개다. 269개는 한국어/legacy 보조 DB에서 ID 후보만 찾았고(번역 표기 변형 후보 포함), 나머지 113개는 ID 후보 미확인이다. 행동 자료 미대조는 385개로, 전수 복원 완료로 표시하지 않는다.
...
| 5 | 검증된 원작 행동 배선 | 기반 필요 | 105개 표본도 공식 과거 KRO/Aegis로 모두 교차 검증된 상태가 아니다. ID와 행동의 출처/신뢰도 gate 뒤 데이터만 연결한다. |
...
1. 행동 미대조 385종(보조 DB ID 후보 293, ID 후보 미확인 92): 한국어 표시명↔legacy `AegisName`/ID 교차자료를 확보하고, 이름 교차와 행동 정본을 분리해 행별 조사한다. legacy `NPCIdentity.lub`는 sprite-name↔ID 보조표로만 쓰며, pinned pre-re DB와 독립 검증 전까지 행동 완료로 판정하지 않는다. Renewal 미러의 `kROName`은 이름 epoch/언어 불일치 때문에 자동 연결에 쓰지 않는다.
...
- ID 후보를 293행으로 갱신했다. 이번 추가 후보는 Dark Priest 1198, Mi Gao 1516(표기 변형), Owl Duke 1320, Garm 1252(동일 Hatii ID의 명칭 variant), Christmas Cookie 1246, Ogretooth 1204(중복 행), Wootan Fighter 1499, Wootan Shooter 1498, Chepet 1250, Nightmare Terror 1379, Bloody Butterfly 1408, Dryad 1493(중복 표기)이다. Mutant Dragon 1262는 Dark Frame 1262와 보조 출처 충돌 때문에 미연결로 유지한다. 이전 추가 후보는 Drainliar 1111, Cruiser 1248, Solider 1316, Freezer 1319, Permeter 1314, Lude 1509, Disguise 1506, Bloody Murderer 1507, Magnolia 1138, Dark Illusion 1302, Stormy Knight 1251, Gig 1387, Breeze 1692, Lord of the Dead 1373이다. 보조 출처 crosswalk일 뿐 행동·시대 검증은 완료되지 않았다. ID 미연결 92행, 행동 자료 미대조 385행이다.
...
- 원작 AI evaluator·주변 개체·groundDrops·원작 Monster Skill 배선·적응형 구역 정책은 구현하지 않았다. 행동 미대조 385종 중 293종은 ID 후보만 있고 92종은 ID 미연결이다.
