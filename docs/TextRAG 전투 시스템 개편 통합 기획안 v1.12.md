# TextRAG 전투 시스템 개편 통합 기획안 v1.12

- 기준 저장소: `J-Roc-H/text-rag`, `main` 기준 커밋 `5451ab1`
- 갱신일: 2026-09-25
- 변경 요약: v1.11 대비 스케골트(청) 1755와 TextRAG 데이터 본체가 세이렌 1634와 일치하는 행을 행동 표에 추가 연결하고, 스콜피온 킹 1168을 ID-only로 기록했다. 행동 대조 466→468, ID 후보만 확인 3→4, ID 미연결 21→18이다. 이그니젬 세니아(검사)라는 표시명은 세이렌과 충돌하므로 명칭 정정 전 배선 대상에서 제외한다.
...
- 몬스터 DB 490행 전부 대장에 열거했다. pinned pre-re 자료 또는 행별 보조출처까지 행동 대조한 행은 468개다. 4개는 canonical ID 후보만 확인했고, 18개는 ID 후보 미확인이다. 행동 자료 미대조는 22개로, 전수 복원 완료로 표시하지 않는다.
...
| 5 | 검증된 원작 행동 배선 | 기반 필요 | 468개는 fixed pre-re 또는 행별 보조 근거까지 대조했지만, 수치·variant 충돌 C 행과 ID-only 4행, 미연결 18행이 남아 있다. 이름과 데이터 본체가 충돌하는 행은 명칭 정정/epoch 확정 전 배선하지 않는다. `Ai` 누락 행은 unknown을 보존하고 확인된 행동만 배선한다. |
...
1. 행동 미대조 22종(ID 후보 4, ID 후보 미확인 18): Angra Mantis 2133, Dark Snake Lord 1418, Timeholder 3074, Scorpion King 1168의 해당 pre-re/한국 과거판을 먼저 찾는다. Peco Peco Egg 1047은 AI Type 누락을 보존한 채 확인된 무반격·고정·idle 변태만 기록 완료다. 나머지는 한국어 표시명↔legacy `AegisName`/ID 교차자료를 확보하며 Renewal 미러의 `kROName`은 자동 연결에 쓰지 않는다.
...
- 고정 revision을 실제 재대조해 35종을 우선 행동 표로 승격한 뒤, 기존 ID 후보 중 TextRAG의 HP·공격범위·공격지연이 R1과 모두 같은 190종을 추가 승격했다. 남은 R1 연결 후보 92종도 HP·범위·공격지연 충돌을 각 행에 기록하고 신뢰도 C로 행동 기계 대조했다. 표제어와 공식 한국 DB를 재대조해 46종을 더 연결하여 현재 행동 표는 468종이다. 스케골트(청)는 한국 DB의 파랑 variant 1755로 교차했으며, 이그니젬 세니아(검사)의 데이터 본체는 세이렌 1634와 전항목 일치하지만 표시명 충돌을 C로 기록했다. 페코페코 알 1047은 AI Type 누락을 유지하면서 R2 idle 변태와 한국 DB의 무반격·고정형을 확인했다. 타임홀더 3074, 흑사왕 1418, 앙그라 만티스 2133, 스콜피온 킹 1168은 ID-only로 남겼다. 현재 ID 후보만 확인된 행은 4, ID 미연결 18, 행동 자료 미대조 22행이다.
...
- 원작 AI evaluator·주변 개체·groundDrops·원작 Monster Skill 배선·적응형 구역 정책은 구현하지 않았다. 행동 미대조 22종 중 4종은 ID 후보만 있고 18종은 ID 미연결이다.
