# TextRAG 전투 시스템 개편 통합 기획안 v1.11

- 기준 저장소: `J-Roc-H/text-rag`, `main` 기준 커밋 `5451ab1`
- 갱신일: 2026-09-25
- 변경 요약: v1.10 대비 표제어·한국 DB·R1 드롭 교차로 7행을 행동 표에 추가 연결하고 타임홀더 3074의 현행 ID를 ID-only로 기록했다. 행동 대조 459→466, ID 후보만 확인 3, ID 미연결 28→21이다. 페코페코 알은 AI Type을 추정하지 않고 확인된 무반격·고정·idle 변태만 B로 기록했으며, Renewal 수치는 정본으로 사용하지 않았다.
...
- 몬스터 DB 490행 전부 대장에 열거했다. pinned pre-re 자료 또는 행별 보조출처까지 행동 대조한 행은 466개다. 3개는 canonical ID 후보만 확인했고, 21개는 ID 후보 미확인이다. 행동 자료 미대조는 24개로, 전수 복원 완료로 표시하지 않는다.
...
| 5 | 검증된 원작 행동 배선 | 기반 필요 | 466개는 fixed pre-re 또는 행별 보조 근거까지 대조했지만, 수치·variant 충돌 C 행과 ID-only 3행, 미연결 21행이 남아 있다. `Ai` 누락 행은 unknown을 보존하고 확인된 행동만 배선한다. |
...
- **타임홀더의 현행 Renewal 스킬을 historical 배선값으로 사용**: ID 3074는 확인됐으나 pinned pre-re 레코드가 없고 HP가 크게 충돌한다. ID-only로 보류한다.
...
1. 행동 미대조 24종(ID 후보 3, ID 후보 미확인 21): Angra Mantis 2133, Dark Snake Lord 1418, Timeholder 3074의 해당 pre-re/한국 과거판을 먼저 찾는다. Peco Peco Egg 1047은 AI Type 누락을 보존한 채 확인된 무반격·고정·idle 변태만 기록 완료다. 나머지는 한국어 표시명↔legacy `AegisName`/ID 교차자료를 확보하며 Renewal 미러의 `kROName`은 자동 연결에 쓰지 않는다.
...
- 고정 revision을 실제 재대조해 35종을 우선 행동 표로 승격한 뒤, 기존 ID 후보 중 TextRAG의 HP·공격범위·공격지연이 R1과 모두 같은 190종을 추가 승격했다. 남은 R1 연결 후보 92종도 HP·범위·공격지연 충돌을 각 행에 기록하고 신뢰도 C로 행동 기계 대조했다. 표제어와 공식 한국 DB를 재대조해 44종을 더 연결하여 현재 행동 표는 466종이다. 새 사례에서 지어스·피트맨·잭은 R1/R2에, 위로하는 자·감시하는 자·지키는 자는 한국 DB 또는 전항목 드롭 교차에 연결했다. 페코페코 알 1047은 AI Type 누락을 유지하면서 R2 idle 변태와 한국 DB의 무반격·고정형을 확인했다. 타임홀더 3074, 흑사왕 1418, 앙그라 만티스 2133은 ID-only로 남겼다. 현재 ID 후보만 확인된 행은 3, ID 미연결 21, 행동 자료 미대조 24행이다.
...
- 원작 AI evaluator·주변 개체·groundDrops·원작 Monster Skill 배선·적응형 구역 정책은 구현하지 않았다. 행동 미대조 24종 중 3종은 ID 후보만 있고 21종은 ID 미연결이다.
