# TextRAG 전투 시스템 개편 통합 기획안 v1.9

- 기준 저장소: `J-Roc-H/text-rag`, `main` 기준 커밋 `5451ab1`
- 갱신일: 2026-09-25
- 변경 요약: v1.8 대비 수치 충돌이 있는 R1 연결 후보 92행도 충돌값을 기록해 행동 기계 대조를 마쳤다. 행동 대조 330→422, ID 후보만 남은 행 94→2, ID 미연결 66행이다. 수치 충돌 92행은 신뢰도 C이며, 역사적 KRO/Aegis epoch 판정과 구현은 별도 단계다.
...
- 몬스터 DB 490행 전부 대장에 열거했다. pinned pre-re 자료까지 행동 대조한 행은 422개다. 2개는 canonical ID 후보만 확인했고, 66개는 ID 후보 미확인이다. 행동 자료 미대조는 68개로, 전수 복원 완료로 표시하지 않는다.
...
| 5 | 검증된 원작 행동 배선 | 기반 필요 | 422개는 fixed pre-re 기계 판독까지 대조했지만, 92개는 수치 충돌 신뢰도 C이며 공식 과거 KRO/Aegis 교차가 전체 행에 끝난 상태는 아니다. ID와 행동의 출처/신뢰도 gate 뒤 데이터만 연결한다. |
...
1. 행동 미대조 68종(ID 후보 2, ID 후보 미확인 66): Angra Mantis 2133의 해당 pre-re/한국 과거판과 Peco Peco Egg 1047의 AI 정의를 먼저 찾고, 남은 66종은 한국어 표시명↔legacy `AegisName`/ID 교차자료를 확보한다. 이름 교차와 행동 정본을 분리하며 Renewal 미러의 `kROName`은 자동 연결에 쓰지 않는다.
...
- 고정 revision을 실제 재대조해 35종을 우선 행동 표로 승격한 뒤, 기존 ID 후보 중 TextRAG의 HP·공격범위·공격지연이 R1과 모두 같은 190종을 추가 승격했다. 남은 R1 연결 후보 92종도 HP·범위·공격지연 충돌을 각 행에 기록하고 신뢰도 C로 행동 기계 대조를 마쳐, 현재 행동 표는 422종이다. Cookie 1265의 Assist·idle heal, Bow Guardian 1830의 idle 원거리 기술·무작위 대상, Ifrit 1832와 Byorgue 1839의 idle 소환, Zombie Master 1298의 idle 소환처럼 최소 세계 상태 요구가 분명한 사례가 포함된다. Dark Frame은 1260, Mutant Dragonoid는 1262로 분리했고, Deathword는 1698, Zombie Master는 1298로 정정했다. 미아비→1516, 노버스(적)→1719, 스콜피온 킹→1168, 흑사왕→1272, 가이아스→1140은 고정 원본과 맞지 않아 후보를 폐기했다. Angra Mantis 2133은 R1 부재, Peco Peco Egg 1047은 R1 AI 부재로 보류했다. 현재 ID 후보만 확인된 행은 2, ID 미연결 66, 행동 자료 미대조 68행이다.
...
- 원작 AI evaluator·주변 개체·groundDrops·원작 Monster Skill 배선·적응형 구역 정책은 구현하지 않았다. 행동 미대조 68종 중 2종은 ID 후보만 있고 66종은 ID 미연결이다.
