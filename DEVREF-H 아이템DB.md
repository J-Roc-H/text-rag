...
### dropSlots 필드 (미설계, 2026-08-13 — DEVREF-E P1-03 배경)

`db-items`에 `dropSlots` 필드가 현재 **0개** 등록 상태 — 드롭 시 슬롯을 부여하는 로직이 4곳 데드코드였고 v9.11에서 `rollDrops()`로 통합되며 방치됨. `slots>0`인 아이템 478종이 대상. "드롭 시 슬롯 부여" 규칙 자체가 아직 설계되지 않음(백로그 진행 중, DEVREF-E P1-03).
