...
- `보물상자`: 원작 동일 항목으로 단정할 수 없는 TextRAG 고유/변형 소모품. 기존 값 유지, 별도 설계 검토.
- `조합 키트`: 원작 동일 항목으로 단정할 수 없는 TextRAG 고유/변형 소모품. 기존 값 유지, 별도 설계 검토.
- `황금포션`: 원작 동일 항목으로 단정할 수 없는 TextRAG 고유/변형 소모품. 기존 값 유지, 별도 설계 검토.

## 완료 조건

- `_aegis` 연결 소모품 87개가 고정 원본 `Weight/10`과 전부 일치한다.
- 화살류 소수 무게 0.1~0.3을 반올림/절삭하지 않는다.
- 보류 3개는 임의로 원작 항목에 매핑하지 않는다.
