...
- **잔여 미분리**: 스킬 쿨다운(`G.cooldowns`, 틱 기반)과 화살 hitEffect proc(위 "화살 hitEffect proc" 참조)은 여전히 ASPD(=processTurn 호출 빈도)에 비례해 가속됨 — 고AGI 캐릭터가 사실상 무적에 가까워지는 문제의 잔여 원인 (백로그 DEVREF-E P1-01)
- **전투 중 자연회복**: 원작 Pre-Re는 전투 중 HP/SP 자연회복이 정지되나 현재 미구현. 위 잔여 미분리와 결합 시 문제가 심화됨 (백로그 DEVREF-E P1-02)
